...
const Cards = ({ cards: propCards }: { cards?: ICardsRender[] }) => {
  const isAuth = useSelector((state: RootStore) => state.user.isAuth)

  const { cartBasket, addBasket, isLoadingProductBasket } = useBasket()
  const { searchQuery, isLoading, cards: contextCards, currentPage } = useProductsContext()
  const { addFavourites, isLoadingProductFav, cartFavourites } = useFavourites()
  const { showNotification } = useUIContextNotification()

  const displayCards = propCards || contextCards

  const cardStates = useMemo(() => {
    return displayCards.reduce((acc: Record<number, ICardState>, card: ICardsRender) => {
      acc[card.id] = {
  ...
  }, [displayCards, cartFavourites, cartBasket, isLoadingProductFav, isLoadingProductBasket])
  ...
  const filteredCards = useMemo(() => 
    displayCards.filter((card: ICardsRender) => 
      card.nazvanie.toLowerCase().includes(searchQuery.toLowerCase())
    ), [displayCards, searchQuery]
  )

  const showEmptyState = 
    !isLoading && (searchQuery && filteredCards.length === 0 || displayCards.length === 0)
        ...
        <>
          {showEmptyState ? <h2 className='centered-heading'>Товары отсутствуют</h2> : (
            <section className={styles.setka}>
              {displayCards.map((card: ICardsRender) => (
                <CardItem
                  key={card.id}
                  card={card}
                  {...cardStates[card.id]}
