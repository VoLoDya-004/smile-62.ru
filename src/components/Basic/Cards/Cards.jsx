import { useState, useEffect, useContext, useRef, useCallback, useMemo, useTransition } from 'react'
...
export default memo(function Cards() {
  ...
  const { cartFavourites, cartBasket } = context
  ...
  async function loadCards() {
    try {
      const response = await axios.get(`http://localhost:3000/src/PHP/pagination.php?page=${currentPage}`)
  ...
  const Card = memo(function Card ({card, isDarkTheme}) {
    ...
    const isAddingBasket = pendingIdBasket === card.id
        ...
        <div className="card__bottom">
          {price === price_sale ? (
            <>
              ...
              >
                {isAddingBasket ? "Добавлени" : "В корзин"}
              </button>
            </>
          ) : (
            <>
