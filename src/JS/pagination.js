    ...
    export default function loadCards() {
      $.ajax({
        url: `http://localhost:3000/src/PHP/pagination.php?page=${currentPage}`,//для локального тестирования
        ...
        success: function(data) {
          ...
          data.forEach(function(obj) {
            ...
            $(document).on("click", "#card_"+obj.id, addIdProduct)

            function addIdProduct2 () {
              addFav(obj.id)
            }
            $(document).on("click", "#card__heart_"+obj.id, addIdProduct2)           
            ...
            `<div id = ${obj.id} class="card">
				      <div class="card__top">
				      <div id="card__heart_${obj.id}" class="card__heart"><svg width="23" height="21" xmlns="http://www.w3.org/2000/svg">
				...
				<div class="card__bottom">
					<div class="card__prices">
						<div class="card__price card__price_discount">${price_sale}</div>
						<div class="card__price card__price_count">${price}</div>
					</div>
					<a href="" class="card__title">${obj.nazvanie}</a>
					<button class="card__btn" id= "card_${obj.id}">В корзину</button>
				</div>
    ...
    let idUser = 222
     $.ajax({
        url: `http://localhost:3000/src/PHP/basket.php`,//для локального тестирования
        //url: `/src/PHP/pagination.php?idProduct=${idProduct}&idUser=222`, //при переносе на хостинг использовать такой адрес
        method: 'GET',
        data: {
          'Operation': 'addBasket',
          'idProduct': idProduct,
    ...
    function addFav(idProduct) {
    let idUser = 222
     $.ajax({
        url: `http://localhost:3000/src/PHP/favourites.php`,//для локального тестирования
        //url: `/src/PHP/pagination.php?idProduct=${idProduct}&idUser=222`, //при переносе на хостинг использовать такой адрес
        method: 'GET',
        data: {
